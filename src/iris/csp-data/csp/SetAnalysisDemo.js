...
var conceptsApp = angular.module('conceptsApp', ['ngSanitize']);
 ...
 conceptsApp.controller('DomainController', ['$scope', function($scope) {
	...
	// retrieve capabilities
   	function getFeatures() {
	     $.ajax({
	     	url: hostURL($scope.ikDomain)+'Features', 
	     	method: 'GET',
 ...
 getUsername = function() { return '_SYSTEM'; };
 getPassword = function() { return 'SYS'; };
 ...
 conceptsApp.controller('ConceptsController', ['$scope', function($scope) {
   ...
   $scope.getSets = function() {
     $scope.selectedSet = '';
     $.ajax({
     	url: hostURL($scope.ikDomain)+'Sets/Info', 
     	method: 'GET',
	  	contentType: 'application/json; charset=UTF-8',
	  	username: getUsername(),
	  	password: getPassword(),
     	complete: function(raw) {
	     	var data = raw.responseJSON;
  ...
  $scope.selectSet =  function(set, index) {
    ...
    $scope.showSourcesBySet(set);
     ...
     $.ajax({
     	url: hostURL($scope.ikDomain)+'Sets/Overlaps/'+set.SetId,
     	method: 'GET',
	  	contentType: 'application/json; charset=UTF-8',
	  	username: getUsername(),
	  	password: getPassword(),
  ...
  $scope.showSourcesBySet = function(set1, set2) {
    if (set2 == undefined) {
	     ...
	     $.ajax({
	     	url: hostURL($scope.ikDomain)+'Sources/Set/'+set1.SetId, 
	     	method: 'GET',
		  	contentType: 'application/json; charset=UTF-8',
		  	username: getUsername(),
		  	password: getPassword(),
	    ...
	    $scope.overlapSourcesRole = 'Overlapping sources for sets \''+set1.Name+'\' and \''+set2.Name+'\'';
	     ...
	     $.ajax({
	     	url: hostURL($scope.ikDomain)+'Sources/Set/'+set1.SetId+'/'+set2.SetId,
	     	method: 'GET',
		  	contentType: 'application/json; charset=UTF-8',
		  	username: getUsername(),
		  	password: getPassword(),
  ...
  $scope.saveSet = function(name) {
   ...
   $scope.refreshConcepts();
	  ...
	  $scope.selectSource = function(srcId) {
	     ...
	     $.ajax({
	     	url: hostURL($scope.ikDomain)+'Sources/'+srcId, 
	     	method: 'GET',
		  	contentType: 'application/json; charset=UTF-8',
		  	username: getUsername(),
		  	password: getPassword(),
 ...
 conceptsApp.controller('SourceController', ['$scope', function($scope) {
	  ...
	  this.getSources = function() {
	     $.ajax({
	     	url: hostURL($scope.ikDomain)+'Sources', 
	     	method: 'GET',
		  	contentType: 'application/json; charset=UTF-8',
		  	username: getUsername(),
		  	password: getPassword(),
